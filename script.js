...
const bankToBoat = (e, i, str) => {
  if (personInBoat < 2) {
    ...
    if (personInBoat === 0) person1.classList.add(`${str}`);
    else {
      if (
        person1.classList.contains("cannibal") ||
        person1.classList.contains("missonary")
      )
        person2.classList.add(`${str}`);
      else person1.classList.add(`${str}`);
    }
    ...
    if (boatPosition === -1) {
      for (let j = 0; j < 3; j++) {
        if (cannibalPosition[j] !== 1) {
          document
            .querySelector(`.${bank}c-${j + 1}`)
            .classList.add("cannibal");
    ...
    if (boatPosition === -1) {
      for (let j = 0; j < 3; j++) {
        if (missonaryPosition[j] !== 1) {
          document
            .querySelector(`.${bank}m-${j + 1}`)
            .classList.add("missonary");
